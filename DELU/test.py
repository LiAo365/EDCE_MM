...
def test(itr, dataset, args, model, device):
    ...
    while not done:
        ...
        with torch.no_grad():
            ...
            # # comment is current best parameter
            # v_mask = (v_atn > 0.015).astype(np.int32)
            # f_mask = (f_atn > 0.015).astype(np.int32)
            ...
            # video_mask = np.abs(v_atn - f_atn) / (v_atn + f_atn + 1e-5) > 0.5
